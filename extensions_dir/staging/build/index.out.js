...
export async function main() {
  ...
  if (!queuedFederated.includes('dbx_text_annotator')) {
    try {
      let ext = require('dbx_text_annotator/lib/plugin');
      for (let plugin of activePlugins(ext)) {
        register.push(plugin);
      }
    } catch (e) {
  ...
  if (!queuedFederated.includes('mlpip_hive')) {
    try {
      let ext = require('mlpip_hive');
      for (let plugin of activePlugins(ext)) {
        register.push(plugin);
      }
    } catch (e) {
